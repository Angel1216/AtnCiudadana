 ...
 public void setSessionFactory(SessionFactory sf) {  
  this.sessionFactory = sf;  
 	...
 	public List<VLlamadaEmergencia> getIncomingCall(){
 		Session session = this.sessionFactory.getCurrentSession();
 		@SuppressWarnings("unchecked")
 		List<VLlamadaEmergencia> lstLlamadaEmergencia = session.createQuery("FROM VLlamadaEmergencia ORDER BY FECHA DESC").list();
 		return lstLlamadaEmergencia.size() > 0 ? lstLlamadaEmergencia: null;
 	}
 	public List<VLlamadaEmergencia> getCallStatus(){
 		Session session = this.sessionFactory.getCurrentSession();
 		@SuppressWarnings("unchecked")
 		List<VLlamadaEmergencia> lstLlamadaStatus = session.createQuery("FROM VLlamadaEmergencia where estatusLlamada IS NOT NULL ORDER BY FECHA DESC")
 		.list();
 		return lstLlamadaStatus.size() > 0 ? lstLlamadaStatus: null;
 	}
}