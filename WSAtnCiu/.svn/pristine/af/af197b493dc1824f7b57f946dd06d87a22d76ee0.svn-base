...
public class ControllerRelPerServ {
	...
	public @ResponseBody
	List<RelPerServ> getRelPerSerByIdUser(
	...
	public @ResponseBody
	List<Integer> getFilesByIdUsuCatServ(
			@PathVariable("rpcIdusucatser") Integer rpcIdusucatser)
			throws JSONException {
	...
	public @ResponseBody
	byte[] getImageByArcIdArchivo(
			@PathVariable("arcIdArchivo") Integer arcIdArchivo)
			throws JSONException {
	...
	public @ResponseBody
	String getImage64ByArcIdArchivo(
			@PathVariable("arcIdArchivo") Integer arcIdArchivo)
			throws JSONException {
	...
	public @ResponseBody
	String getImage64SizeByArcIdArchivo(
			@PathVariable("arcIdArchivo") Integer arcIdArchivo,
			@PathVariable("width") Integer width,
			@PathVariable("height") Integer height) throws JSONException {
		return serviceReport.getImage64SizeByArcIdArchivo(arcIdArchivo, width,
				height);
	}

	@RequestMapping(value = "/getRelPerSerByIdService/{flIdService}", method = RequestMethod.GET, headers = "Accept=application/json")
	public @ResponseBody
	List<RelPerServ> getRelPerSerByIdService(
			@PathVariable("flIdService") Integer flIdService)
			throws JSONException {
		ServicioPadre objServicioPadre = new ServicioPadre(flIdService);
		List<RelPerServ> list = serviceReport
				.getRelPerSerByIdService(objServicioPadre);
		System.out.println("list: " + list);
		return list;
	}

	@RequestMapping(value = "/getRelPerSerAlert", method = RequestMethod.POST, headers = "Accept=application/json")
	public String getRelPerSerAlert(@RequestBody RelPerServ objRelPerServ)
			throws JSONException {
	...
	public void getRelPerSerArchivos(@RequestBody Archivos objArchivos)
			throws JSONException {
		serviceReport.getRelPerSerArchivos(objArchivos);
	}

	@RequestMapping(value = "/getUltimoFolio/{flNomenclatura}", method = RequestMethod.GET)
	public String getUltimoFolio(@PathVariable String flNomenclatura) {
		String flFolio = serviceReport.getUltimoFolio(flNomenclatura);
		if (flFolio != null) {
			return flFolio;
		} else {
			return null;
	...
	public RelPerServ getRelPerSerById(
			@PathVariable("idRelPerServ") Integer idRelPerServ) {
	...
	@RequestMapping(value = "/deleteReports/{idReport}", method = RequestMethod.PUT)
	public void deleteReports(@PathVariable Integer idReport) {
		serviceReport.deleteReports(idReport);
	}
